            ...
            && let Event::Key(key) = event::read()? {
                match app.screen {
                    ...
                    AppScreen::NewCampaign => match key.code {
                        ...
                        crossterm::event::KeyCode::Enter => {
                            if !app.new_campaign_name.is_empty()
                                && !app.new_campaign_symbol.is_empty()
                            {
                                let target_price =
                                    app.new_campaign_target_price.parse::<f64>().ok();
                                Campaign::insert(
                                    &app.db_conn,
                    ...
                    AppScreen::EditTrade => match key.code {
                        ...
                        crossterm::event::KeyCode::Enter => {
                            if let Some(trade_id) = app.edit_trade_id {
                                ...
                                let date_fmt = format_description!("[year]-[month]-[day]");
                                let expiration_date =
                                    Date::parse(&app.edit_trade_fields[4], &date_fmt)
                                        .unwrap_or_else(|_| {
                                            time::OffsetDateTime::now_local().unwrap().date()
                                        });
                                let date_of_action =
                                    Date::parse(&app.edit_trade_fields[5], &date_fmt)
                                        .unwrap_or_else(|_| {
                                            time::OffsetDateTime::now_local().unwrap().date()
                                        });
