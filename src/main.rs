...
impl Campaign {
    ...
    fn insert(conn: &Connection, name: &str, symbol: &str, target_exit_price: Option<f64>) -> Option<Campaign> {
        ...
        Some(Campaign { id: id as i32, name: name.to_string(), symbol: symbol.to_string(), created_at: now, target_exit_price })
    }
}
...
const ACTIONS: [&str; 6] = ["BuyPut", "SellPut", "BuyCall", "SellCall", "Exercised", "Assigned"];
...
impl App {
    ...
    fn reload_campaigns(&mut self) {
        ...
        self.campaign_list_state.select(Some(self.campaign_select_index));
    }
...
fn calculate_campaign_summary(trades: &[&OptionTrade], target_exit_price: Option<f64>) -> (Option<f64>, i32, Option<f64>, f64, f64) {
    // Break-even calculation
    let total_debits: f64 = trades.iter()
        .filter(|t| matches!(t.action, Action::Assigned | Action::BuyCall | Action::BuyPut))
        .map(|t| t.credit * t.number_of_shares as f64)
        .sum();
    
    let total_credits: f64 = trades.iter()
        .filter(|t| matches!(t.action, Action::SellPut | Action::SellCall))
        .map(|t| t.credit * t.number_of_shares as f64)
        .sum();
    
    let total_shares_assigned: i32 = trades.iter()
    ...
    let first_trade_date = trades.iter()
        .map(|t| t.date_of_action)
        .min();
    ...
    let profit_per_week = if let Some(target_price) = target_exit_price {
        if total_shares_assigned > 0 && weeks_running > 0 {
            let target_profit = (target_price - break_even.unwrap_or(0.0)) * total_shares_assigned as f64;
    ...
    (break_even, weeks_running, profit_per_week, total_credits, running_profit_loss)
}
            ...
            if let Event::Key(key) = event::read()? {
                match app.screen {
                    AppScreen::CampaignSelect => match key.code {
                        ...
                        KeyCode::Down => {
                            if app.campaign_select_index + 1 < app.campaigns.len() {
                                ...
                                app.campaign_list_state.select(Some(app.campaign_select_index));
                            }
                        },
                        KeyCode::Up => {
                    ...
                    AppScreen::NewCampaign => match key.code {
                        ...
                        KeyCode::Enter => {
                            if !app.new_campaign_name.trim().is_empty() && !app.new_campaign_symbol.trim().is_empty() {
                                let target_price = app.new_campaign_target_price.trim().parse().ok();
                                if let Some(camp) = Campaign::insert(&app.db_conn, &app.new_campaign_name, &app.new_campaign_symbol, target_price) {
                        ...
                        KeyCode::Backspace => {
                            match app.new_campaign_field {
                                0 => { app.new_campaign_name.pop(); },
                                1 => { app.new_campaign_symbol.pop(); },
                                2 => { app.new_campaign_target_price.pop(); },
                                _ => {}
                            }
                        },
                        _ => {}
                    ...
                    AppScreen::AddTrade => match key.code {
                        ...
                        KeyCode::Tab => app.form_index = (app.form_index + 1) % (app.form_fields.len() + 1),
                        KeyCode::BackTab => app.form_index = (app.form_index + app.form_fields.len()) % (app.form_fields.len() + 1),
                        ...
                        KeyCode::Enter => {
                            if let Some(trade) = try_build_trade(&app.form_fields, app.action_index, app.selected_campaign.as_ref().unwrap()) {
                                let _ = trade.insert(&app.db_conn);
                                app.reload_trades();
                                app.screen = AppScreen::CampaignDashboard;
                                app.form_error = None;
                            } else {
                                app.form_error = Some("Invalid input. Please check all fields.".to_string());
                            }
                        },
                        KeyCode::Char(c) if app.form_index != 1 => {
                            app.form_fields[if app.form_index < 1 { app.form_index } else { app.form_index - 1 }].push(c);
                        },
                        KeyCode::Backspace if app.form_index != 1 => {
                            app.form_fields[if app.form_index < 1 { app.form_index } else { app.form_index - 1 }].pop();
                        },
                        _ => {}
                    },
                    ...
                    AppScreen::EditTrade => match key.code {
                        ...
                        KeyCode::Char(c) if app.edit_form_index != 1 => {
                            app.edit_trade_fields[if app.edit_form_index < 1 { app.edit_form_index } else { app.edit_form_index - 1 }].push(c);
                        },
                        KeyCode::Backspace if app.edit_form_index != 1 => {
                            app.edit_trade_fields[if app.edit_form_index < 1 { app.edit_form_index } else { app.edit_form_index - 1 }].pop();
                        },
                        KeyCode::Enter => {
                            if let Some(id) = app.edit_trade_id {
                                if let Some(trade) = try_build_trade_edit(&app.edit_trade_fields, app.edit_action_index, app.selected_campaign.as_ref().unwrap().name.as_str(), id) {
...
fn draw_main_menu(f: &mut Frame) {
    let size = f.area();
    let block = Block::default().title("Options Tracker").borders(Borders::ALL);
    let items = vec![
...
fn draw_campaign_select(f: &mut Frame, app: &App) {
    let size = f.area();
    let block = Block::default().title("Select Campaign [n: new, ↑/↓: move, Enter: select, q: quit]").borders(Borders::ALL);
    let items: Vec<ListItem> = app.campaigns.iter().map(|c| ListItem::new(c.name.clone())).collect();
    let list = List::new(items).block(block).highlight_symbol("> ");
...
fn draw_new_campaign(f: &mut Frame, app: &App) {
    let size = f.area();
    let block = Block::default().title("New Campaign [Tab: switch, Enter: save, ESC: cancel]").borders(Borders::ALL);
    let name_focus = if app.new_campaign_field == 0 { " <" } else { "" };
    let symbol_focus = if app.new_campaign_field == 1 { " <" } else { "" };
    let price_focus = if app.new_campaign_field == 2 { " <" } else { "" };
    let content = format!("Name: {}{}\nSymbol: {}{}\nTarget Exit Price: {}{}", app.new_campaign_name, name_focus, app.new_campaign_symbol, symbol_focus, app.new_campaign_target_price, price_focus);
...
fn draw_campaign_dashboard(f: &mut Frame, app: &App) {
    ...
    let title = if let Some(camp) = &app.selected_campaign {
        format!("Campaign: {} [a: add trade, v: view trades, ESC: back]", camp.name)
    } else {
    ...
    let campaign_trades: Vec<&OptionTrade> = app.trades.iter()
        .filter(|t| t.campaign == app.selected_campaign.as_ref().unwrap().name && t.symbol == app.selected_campaign.as_ref().unwrap().symbol)
        .collect();
    
    let (break_even, weeks_running, profit_per_week, total_credits, running_profit_loss) = calculate_campaign_summary(&campaign_trades, app.selected_campaign.as_ref().unwrap().target_exit_price);
    let pl_color = if running_profit_loss >= 0.0 { Color::Green } else { Color::Red };
    let summary_lines = vec![
        Line::from(vec![Span::raw(""),]),
        Line::from(vec![Span::styled("Campaign Summary:", Style::default().add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::raw(format!(
            "Target Exit Price: {}",
            app.selected_campaign.as_ref().unwrap().target_exit_price.map(|p| format!("${:.2}", p)).unwrap_or_else(|| "N/A".to_string())
        ))]),
        Line::from(vec![Span::raw(format!("Total Credits: ${:.2}", total_credits))]),
        Line::from(vec![
        ...
        Line::from(vec![Span::raw(format!(
            "Break Even: {}",
            break_even.map(|be| format!("${:.2}", be)).unwrap_or_else(|| "N/A".to_string())
        ))]),
        Line::from(vec![Span::raw(format!("Weeks Running: {}", weeks_running))]),
        Line::from(vec![Span::raw(format!(
            "Profit per Week: {}",
            profit_per_week.map(|ppw| format!("${:.2}", ppw)).unwrap_or_else(|| "N/A".to_string())
        ))]),
    ];
...
fn draw_view_trades(f: &mut Frame, app: &App) {
    let size = f.area();
    let block = Block::default().title("View Trades [Up/Down: scroll, ESC: return]").borders(Borders::ALL).style(Style::default().fg(Color::Cyan));
    ...
    let widths = [
        Constraint::Length(8), Constraint::Length(12), Constraint::Length(8), Constraint::Length(7),
        Constraint::Length(6), Constraint::Length(12), Constraint::Length(12), Constraint::Length(6), Constraint::Length(7), Constraint::Length(12)
    ];
...
fn draw_edit_trade(f: &mut Frame, app: &mut App) {
    let size = f.area();
    let block = Block::default().title("Edit Trade [Tab: next, Shift+Tab: prev, ←/→: change action, Enter: save, ESC: cancel]").borders(Borders::ALL);
    let fields = [
        "Symbol", "Action", "Strike", "Delta", "Expiration (YYYY-MM-DD)", "Date of Action (YYYY-MM-DD)", "Shares", "Credit"
    ];
    let items: Vec<ListItem> = fields.iter().enumerate().map(|(i, label)| {
...
fn try_build_trade(fields: &[String; 6], action_index: usize, campaign: &Campaign) -> Option<OptionTrade> {
    use Action::*;
...
fn try_build_trade_edit(fields: &[String; 8], action_index: usize, campaign: &str, id: i32) -> Option<OptionTrade> {
    use Action::*;
    let action = match ACTIONS[action_index] {
        "BuyPut" => BuyPut,
        "SellPut" => SellPut,
